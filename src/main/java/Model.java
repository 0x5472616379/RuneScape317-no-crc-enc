...
public class Model extends Entity {
    ...
    public static void init(int count, OnDemand ondemand) {
        ...
        newmodel = new boolean[count + 80000];
...
//        }
    ...
    public void read622Model(byte abyte0[], int modelID) {
        ...
        vertexLabel = new int[numVertices];
        faceInfo = new int[numTriangles];
        facePriority = new int[numTriangles];
        faceAlpha = new int[numTriangles];
        ...
        for (int i12 = 0; i12 < numTriangles; i12++) {
            ...
            boolean compatHiddenFace = false;
            if (l1 == 1) {
                int info = nc2.read8();
                if (info == 2) {
                    triangleColours2[i12] = 65535;
                    info = 0;
                    compatHiddenFace = true;
                }
                faceInfo[i12] = info;
            }
            if (i2 == 255) {
                facePriority[i12] = nc3.read8();
            }
            if (j2 == 1) {
                faceAlpha[i12] = nc4.read8();
                if (faceAlpha[i12] < 0)
                    faceAlpha[i12] = (256 + faceAlpha[i12]);
            }
            if (compatHiddenFace && faceAlpha != null) {
                faceAlpha[i12] = 255;
            }
            if (k2 == 1)
                faceLabel[i12] = nc5.readU8();
            if (l2 == 1)
                textureIds[i12] = (short) (nc6.readU16() - 1);
            if (textureCoordinates != null)
                if (textureIds[i12] != -1)
                    textureCoordinates[i12] = texture_coordinates[i12] = (byte) (nc7.readU8() - 1);
                else
        ...
        for (int k14 = 0; k14 < numTexTriangles; k14++) {
            int i15 = O[k14] & 0xff;
            if (i15 == 0) {
                texTrianglesPoint1[k14] = nc1.readU16();
                texTrianglesPoint2[k14] = nc1.readU16();
                texTrianglesPoint3[k14] = nc1.readU16();
            }
            if (i15 == 1) {
                texTrianglesPoint1[k14] = nc2.readU16();
                texTrianglesPoint2[k14] = nc2.readU16();
                texTrianglesPoint3[k14] = nc2.readU16();
                if (newformat < 15) {
                    kb[k14] = nc3.readU16();
                    if (newformat >= 14)
                        N[k14] = nc3.read24();
                ...
                gb[k14] = (byte) nc4.readU8();
                lb[k14] = (byte) nc5.readU8();
                F[k14] = (byte) nc6.readU8();
            }
            if (i15 == 2) {
                texTrianglesPoint1[k14] = nc2.readU16();
                texTrianglesPoint2[k14] = nc2.readU16();
                ...
                lb[k14] = (byte) nc5.readU8();
                F[k14] = (byte) nc6.readU8();
                cb[k14] = (byte) nc6.readU8();
                J[k14] = (byte) nc6.readU8();
            }
            if (i15 == 3) {
                texTrianglesPoint1[k14] = nc2.readU16();
                texTrianglesPoint2[k14] = nc2.readU16();
    ...
    public void convertTexturesTo317(short[] textureIds, int[] texa, int[] texb, int[] texc, boolean osrs) {
        ...
        if (textureIds != null) {
            ...
            for (int i = 0; i < faceCount; i++) {
                ...
                texturedVertexC[set++] = texture_type == -1 ? c : texc[texture_type];
    ...
    public void recolor(int src, int dst) {
        int src16 = src & 0xFFFF;
        for (int k = 0; k < faceCount; k++) {
            if ((faceColor[k] & 0xFFFF) == src16) {
                faceColor[k] = dst;
            }
        }
    }

    public int countFacesWithColor(int color) {
        int count = 0;
        for (int f = 0; f < faceCount; f++) {
            if ((faceColor[f] & 0xFFFF) == (color & 0xFFFF)) {
                count++;
            }
        }
        return count;
    ...
    public int countTexturedFacesWithColor(int color) {
        int count = 0;
        for (int f = 0; f < faceCount; f++) {
            boolean textured = (faceInfo == null) || ((faceInfo[f] & 2) == 2);
            if (textured && ((faceColor[f] & 0xFFFF) == (color & 0xFFFF))) {
                count++;
            }
        }
        return count;
     ...
     */
    private boolean hasValidTexturedFace(int texturedFace) {
        if (texturedFace < 0 || texturedFace >= texturedFaceCount) {
            return false;
        }
        if (texturedVertexA == null || texturedVertexB == null || texturedVertexC == null) {
            return false;
        }
        if (texturedFace >= texturedVertexA.length || texturedFace >= texturedVertexB.length || texturedFace >= texturedVertexC.length) {
            return false;
        }
        int ta = texturedVertexA[texturedFace];
        int tb = texturedVertexB[texturedFace];
        int tc = texturedVertexC[texturedFace];
        return ta >= 0 && tb >= 0 && tc >= 0 && ta < vertexCount && tb < vertexCount && tc < vertexCount;
    }
    ...
    public void drawFace(int face) {
        ...
        if (type == 0) {
            ...
            if (!hasValidTexturedFace(texturedFace)) {
                Draw3D.fillGouraudTriangle(vertexScreenY[a], vertexScreenY[b], vertexScreenY[c], vertexScreenX[a], vertexScreenX[b], vertexScreenX[c], faceColorA[face], faceColorB[face], faceColorC[face]);
                return;
            }
            int ta = texturedVertexA[texturedFace];
